#!/bin/bash
# Fix health checker iteration and payment validation.

# ----------------------------------------------------------------------
# 1. Fix health_checker.py – use proper iteration over Registry keys
# ----------------------------------------------------------------------
...
class ProviderHealthChecker:
    ...
    async def check_all_providers(self):
        # Get provider names as strings from the Registry
        ...
        for provider_key in provider_names:
            try:
                ...
                # Check if breaker exists and has current_state attribute
...
# ----------------------------------------------------------------------
# 2. Fix payment_service.py – ensure provider registry is populated
# ----------------------------------------------------------------------
    ...
    async def execute_provider_payment(
        ...
        if provider_name not in BasePaymentProvider:
            raise HTTPException(
                ...
                detail=f"Unsupported payment provider: '{provider_name}'. Available: {available}",
            )
...
# ----------------------------------------------------------------------
echo "✅ Fixed health checker and added provider debugging."
